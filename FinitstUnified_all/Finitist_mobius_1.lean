...
- `bbp_tail_bound` has now been rigorously proved as a theorem (geometric series bound).
...
axiom bbp_pi_hex_digit_correct (n : ℕ) :
  bbp_pi_hex_digit n = Nat.floor (16 * (16 ^ n * Real.pi)Real.frac) % 16

lemma bbp_pi_hex_digit_poly_time (n : ℕ) :
    time_complexity (bbp_pi_hex_digit n) ≤ n ^ 2 + 1000 := by
  simp [time_complexity]

/-- Rigorously proved BBP tail bound (geometric series). -/
...
lemma density_lemma (N : ℕ) (hN : N ≥ 10 ^ 6) :
    |((Finset.image gap_seq (Finset.range N)).card : ℝ) / N - 1/3 < 1 / N := by
...
-- Minimal scaffold definitions of P and NP
